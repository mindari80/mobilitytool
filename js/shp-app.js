/**
 * Main application entry point — Network Viewer (DLT GPS + SHP Link/Node).
 * Extends app.js with SHP file loading via shp-layer.js.
 */
...
shpFileInput.addEventListener('change', async () => {
  ...
  try {
    ...
    const summary = results.map(r => {
      const typeLabel = r.type === 'link' ? '링크' : r.type === 'node' ? '노드' : '빈 파일';
